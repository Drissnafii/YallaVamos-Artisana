...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden" style="background: linear-gradient(to bottom, white, #fff7ed);">
        ...
        <form action="{{ route('admin.matches.update', $match) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="date" id="date"
                        ...
                        class="form-input rounded-md shadow-sm w-full @error('date') border-red-500 @enderror" required>
                    @error('date')
            ...
            <div id="score-section" class="grid grid-cols-1 md:grid-cols-2 gap-6" style="{{ ($match->status == 'in_progress' || $match->status == 'completed') ? 'display: grid;' : 'display: none;' }}">
                <div>
                    <label for="score_team1" class="block text-sm font-medium text-gray-700 mb-1">Team 1 Score</label>
                    <input type="number" name="score_team1" id="score_team1" value="{{ old('score_team1', $match->score_team1 ?? 0) }}" min="0" class="form-input rounded-md shadow-sm w-full @error('score_team1') border-red-500 @enderror">
                    @error('score_team1')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="score_team2" id="score_team2" value="{{ old('score_team2', $match->score_team2 ?? 0) }}" min="0" class="form-input rounded-md shadow-sm w-full @error('score_team2') border-red-500 @enderror">
                    @error('score_team2')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="pt-4 border-t border-gray-200">
                <div class="flex justify-end">
                    ...
                    <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white font-medium py-2 px-4 rounded shadow">Update Match</button>
                </div>
            </div>
        </form>
    </div>
