...
<div class="max-w-5xl mx-auto relative">
    <!-- Profile header -->
    <div class="rounded-lg overflow-hidden mb-8 relative" style="z-index: 9999;">
        <div class="bg-gradient-to-r from-gray-800 to-gray-900 px-6 py-12 sm:px-10 relative">
            ...
            @if($user->background_image)
                <img src="{{ Storage::url($user->background_image) }}" alt="Profile Background"
            ...
            @else
                <img src="{{ asset('images/world-cup-pattern.png') }}" alt="World Cup Pattern"
                    class="absolute inset-0 object-cover mix-blend-overlay opacity-20 w-full h-full">
            @endif

            <!-- Background image upload button -->
            <label for="background_image"
                class="absolute top-3 right-3 p-2 bg-black bg-opacity-50 rounded-full text-white cursor-pointer hover:bg-opacity-70 transition-all duration-200" style="z-index: 10000;">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-6 relative" style="z-index: 10000;">
                <!-- Profile image with Material-style elevation -->
                <div class="relative group">
                    <div
                        class="w-32 h-32 sm:w-40 sm:h-40 rounded-full bg-white overflow-hidden shadow-lg transition-transform hover:scale-105">
                    ...
                    <label for="profile_photo"
                        class="absolute inset-0 flex items-center justify-center rounded-full bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 transition-all duration-200 cursor-pointer">
    ...
    @if (session('status'))
        <div id="status-alert"
            class="mb-6 rounded-lg bg-green-100 border-l-4 border-green-500 p-4 transition-all duration-500 ease-in-out shadow-md relative z-0">
            <div class="flex items-center">
                ...
                <div class="ml-auto pl-3">
                    <div class="-mx-1.5 -my-1.5">
                        <button type="button" onclick="document.getElementById('status-alert').remove()"
                            class="inline-flex rounded-md p-1.5 text-green-500 hover:bg-green-200 focus:outline-none">
    ...
    <!-- Profile edit form -->
    <form method="POST" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" class="relative z-0">
        @csrf
        @method('PATCH')
            ...
            <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
                ...
                <button type="submit" class="
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20"
                        fill="currentColor">
                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 
                ...
                backgroundInput.addEventListener('change', function (e) {
                    if (e.target.files && e.target.files[0]) {
                        ...
                        reader.onload = function (event) {
                            ...
                            const existingBackgroundImages = backgroundPreview.querySelectorAll('img:not(#profile_photo_preview)');
                            existingBackgroundImages.forEach(img => img.remove());

                            // Set the new background image
                            backgroundPreview.style.backgroundImage = `url(${event.target.result})`;
                            backgroundPreview.style.zIndex = '0';
                        }
