...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden" style="background: linear-gradient(to bottom, white, #f5f3ff);">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Team Information</h2>
        </div>

        <form action="{{ route('admin.teams.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code') }}" maxlength="3" class="form-input rounded-md shadow-sm w-full @error('code') border-red-500 @enderror" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="is_qualified" id="is_qualified" class="form-select rounded-md shadow-sm w-full @error('is_qualified') border-red-500 @enderror">
                        <option value="1" {{ old('is_qualified', '1') == '1' ? 'selected' : '' }}>Qualified</option>
                        <option value="0" {{ old('is_qualified') == '0' ? 'selected' : '' }}>Pending</option>
                    </select>
                    @error('is_qualified')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Upload a flag image for the team (JPEG, PNG, or GIF).</p>
                @error('flag')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="pt-4 border-t border-gray-200">
                <div class="flex justify-end">
                    ...
                    <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white font-medium py-2 px-4 rounded shadow">Create Team</button>
                </div>
            </div>
        </form>
    </div>
        ...
        if (nameInput && codeInput && codeInput.value === '') {
            nameInput.addEventListener('blur', function() {
                if (codeInput.value === '') {
                    ...
                    if (name) {
                        // Generate 3-letter code from first 3 letters of name
