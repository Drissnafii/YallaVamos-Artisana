...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Manage Stadiums</h1>
        <a href="{{ route('admin.stadiums.create') }}" class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded transition duration-150 ease-in-out flex items-center">
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden" style="background: linear-gradient(to bottom, white, #f0fdf4);">
        <div class="p-4 border-b">
            <form action="{{ route('admin.stadiums.index') }}" method="GET" class="flex items-center">
                <input type="text" name="search" placeholder="Search stadiums..." value="{{ request('search') }}"
                    class="border rounded-lg px-3 py-2 w-full md:w-80 focus:outline-none focus:ring-2 focus:ring-green-500">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200" style="background: linear-gradient(to bottom, white, #f0fdf4);">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a href="{{ route('admin.stadiums.show', $stadium) }}" class="text-blue-600 hover:text-blue-900 mr-3">
                            View
                        </a>
                        <a href="{{ route('admin.stadiums.edit', $stadium) }}" class="text-green-600 hover:text-green-900 mr-3">
                            Edit
                        </a>
                        <form action="{{ route('admin.stadiums.destroy', $stadium) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this stadium?')">
                                Delete
                            </button>
                        </form>
                    </td>
                ...
                <tr>
                    <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                        No stadiums found. <a href="{{ route('admin.stadiums.create') }}" class="text-green-600 hover:text-green-900">Create your first stadium</a>.
                    </td>
                </tr>
                @endforelse
            </tbody>
